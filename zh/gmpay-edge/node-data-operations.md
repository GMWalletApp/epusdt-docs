...
发布镜像内置同一个 CLI。停止服务后，挂载外部备份目录并执行：
...
输入或输出路径必须显式指定，备份输出还必须位于数据目录之外。恢复只接受全新或空目标目录，不会覆盖非空目录。清单会校验每个文件；恢复还会执行 SQLite 完整性、外键和迁移校验和检查。
...
实例没有对象时可以省略 `--r2-dir`。`--r2-manifest` 也是可选参数，但必须与 `--r2-dir` 一起使用；它用于保留 R2 HTTP 元数据和自定义元数据。该文件是一个以原始 R2 Object Key 为键的 JSON 对象：
